...
class ProductListViewTest(TestCase):
    def setUp(self):
        ...
        Product.objects.bulk_create([
            ...
            Product(
                id            = 2,
                en_name       = 'Jordan 1 x Travis Scott x Fragment Retro Low OG SP Military Blue',
                ko_name       = '조던 1 x 트래비스 스캇 x 프라그먼트 레트로 로우 OG SP 밀리터리 블루',
                model_number  = 'DM7866-140',
                color         = 'SAIL/BLACK/MILITARY BLUE',
                release_date  = '2021-08-13',
                release_price = 189000,
                category_id   = 4,
                brand_id      = 1,
                fast_shipping = False
            ),
    ...
    def test_productlist_get_success(self):
        client   = Client()
        response = client.get('/products?keyword=jordan')
    ...
    def test_productlist_fail(self):
        client = Client()
        response = client.get('/products?sort=아무거나')
